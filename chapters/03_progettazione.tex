...
\indent
\textbf{Session}(\underline{\texttt{session\_id}}, \texttt{title}*, \texttt{last\_domain}*, \texttt{created\_at})\\
\indent\indent vincolo di tupla: \texttt{title} is NULL $\Rightarrow $ \texttt{title} = ``New Chat''\\
\indent\indent vincolo di tupla: \texttt{last\_domain} is NULL $\Rightarrow$ \texttt{last\_domain} = ``''\\
\indent\indent vincolo di tupla: \texttt{last\_domain} is not NULL $\Rightarrow$ \texttt{last\_domain} $\in$ \{``www.marvel.com'',\\ 
\indent\indent\indent\indent\indent\indent\,\,\,\, ``www.ipsos.com'', ``www.raiplaysound.com'', ``it.wikipedia.org'',\\
\indent\indent\indent\indent\indent\indent\,\,\,\, ``*''\}\\
\indent
\textbf{Message}(\underline{\texttt{id}}, \texttt{role}, \texttt{content}, \texttt{created\_at})\\
\indent\indent foreign key: Message[\texttt{id}] $\subseteq$ Contains[\texttt{message}]\\
\indent\indent vincolo di tupla: \texttt{role} $\in$ \{``user'', ``assistant''\}\\
\indent
\textbf{ParsedURL}(\underline{\texttt{id}}, \texttt{url}, \texttt{parsed\_text}*, \texttt{created\_at})\\
\indent\indent chiave: \texttt{url}\\
\indent\indent vincolo esterno: \texttt{parsed\_text} is not NULL $\Rightarrow$ \texttt{parsed\_text} is in MD format\\
\indent\indent vincolo esterno: \texttt{url} is a valid URL\\ 
\indent
\textbf{Contains}(\underline{\texttt{message}}, \texttt{session})\\
\indent\indent foreign key: Contains[\texttt{message}] $\subseteq$ Message[\texttt{id}]\\
\indent\indent foreign key: Contains[\texttt{session}] $\subseteq$ Session[\texttt{session\_id}]\\
\indent\indent vincolo esterno: $(m, s) \in \text{Contains} \Rightarrow \text{\textit{timestamp}}(m) > \text{\textit{timestamp}}(s)$\\

Si noti infine che si è scelto di assegnare, per comodità, opportuni valori di default agli attributi \texttt{title} e \texttt{last\_domain} della tabella \texttt{Session}.
...
\indent\indent\indent\indent\indent\indent\,\,\,\, ``*''\}\\
\indent
\textbf{Message}(\underline{\texttt{id}}, \texttt{session}, \texttt{role}, \texttt{content}, \texttt{created\_at})\\
\indent\indent foreign key: Message[\texttt{session}] $\subseteq$ Session[\texttt{session\_id}]\\
\indent\indent vincolo di tupla: \texttt{role} $\in$ \{``user'', ``assistant''\}\\
\indent\indent vincolo esterno: $m \in \text{Message} \Rightarrow \text{m.\texttt{created\_at}} > \text{\textit{timestamp}}(\text{m.\texttt{session}})$\\
\indent
\textbf{ParsedURL}(\underline{\texttt{id}}, \texttt{url}, \texttt{parsed\_text}*, \texttt{created\_at})\\
\indent\indent chiave: \texttt{url}\\
\indent\indent vincolo esterno: \texttt{parsed\_text} is not NULL $\Rightarrow$ \texttt{parsed\_text} is in MD format\\
