...
Il framework usufruisce di due librerie fondamentali quali Starlette, un toolkit web ASGI (\textit{Asynchronous Server Gateway Interface}) per la gestione di servizi asincroni, e Pydantic, grazie a cui è possibile una validazione automatica e personalizzabile dei dati e la creazione di modelli Pydantic per la definizione dei tipi di input e output di ciascun \textit{endpoint}\footnote{endpoint: nell'ambito delle API, un URL specifico tramite cui un'applicazione riceve ed elabora richieste API}.
...
Per permettere a reti neurali complesse di funzionare su hardware \textit{consumer}, Ollama implementa un processo noto come quantizzazione dei modelli. Questa tecnica riduce la precisione matematica dei pesi dell'LLM, abbattendo pertanto in maniera drastica la quantità di RAM nonché la potenza di calcolo richieste per l'esecuzione del modello.
...
In secondo luogo, l'effettiva velocità di esecuzione di Ollama è rigidamente vincolata all'hardware della macchina ospitante, prediligendo architetture basate su GPU (\textit{Graphics Processing Unit}) a quelle CPU (\textit{Central Processing Unit}). Naturalmente, l'esecuzione di un LLM tramite Ollama è comunque possibile su macchine sprovviste di GPU, a patto di accettare un'inferenza più lenta del modello.
