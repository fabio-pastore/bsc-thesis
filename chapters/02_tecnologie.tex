...
Ollama~\cite{ollama_docs} è un framework open source ideato per semplificare l'esecuzione, la configurazione e la gestione locale degli LLM su svariati sistemi operativi come macOS, Windows e Linux.
...
Si noti per di più che, poiché i modelli sono eseguiti interamente in locale, il sistema non trasmette alcuna informazione a servizi esterni, garantendo quindi privacy e indipendenza da connessioni internet o provider di terza parte (\textit{third-party}) all'utente che ne usufruisce.

Inoltre, giacché l'elaborazione, necessaria all'inferenza del modello, grava direttamente sull'hardware dell'utente, il costo derivante dall'interazione con l'LLM diviene fondamentalmente trascurabile.

In aggiunta a ciò, il framework supporta e aggiorna assiduamente una libreria di modelli pubblici, tra cui Llama, Mistral, Gemma e Phi-3, permettendo di selezionare il modello più adatto alle proprie necessità.
...
Per permettere a reti neurali complesse di funzionare su hardware non particolarmente performante, Ollama implementa un processo noto come quantizzazione dei modelli. Questa tecnica consiste nella riduzione della precisione matematica dei pesi dell'LLM, abbattendo pertanto notevolmente la quantità di RAM nonché la potenza di calcolo richieste per l'esecuzione del modello.
...
In secondo luogo, l'effettiva velocità di esecuzione di Ollama è rigidamente vincolata, come già detto, all'hardware della macchina ospitante, prediligendo peraltro architetture basate su GPU (\textit{Graphics Processing Unit}) a quelle CPU (\textit{Central Processing Unit}), per ovvi motivi. Naturalmente, l'esecuzione di un LLM tramite Ollama è comunque possibile su macchine sprovviste di GPU, a patto di accettare un'inferenza più lenta del modello.
